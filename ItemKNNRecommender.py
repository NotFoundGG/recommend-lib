...
@LastEditors: Yudi
@LastEditTime: 2019-11-14 10:48:02
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sim_method', 
                        type=str, 
                        default='cosine', 
                        help='method to calculate similarity, options for cosine, jaccard, pearson')
    ...
    # cv for 5-fold cv, fo for ratio split randomly, tfo for split by timestamp
    # loo for leave-one-out split, tloo for leave-one-out by timestamp
    ...
    df = load_rate(args.dataset, prepro=args.prepro)
    ...
    for i in range(len(train_set_list)):
        ...
        algo = KNNWithMeans(user_num, item_num, args.k, args.mink, sim_options=sim_options)
        algo.fit(train_set_list[i])
